...
public abstract class TimingExperiment {
  ...
  /**
   * Abstract method for setting up the experiment infrastructure for a given problem size.
   *
  ...
  public void run() {
    for (int i = 0; i < this.problemSizes.size(); i++) {
      ...
      long medianTime = this.computeElapsedTime(this.problemSizes.get(i));
  ...
  /**
   * Convert the timing experiment results to a string that's in CSV format.
   */
  public String toCSVString(){
    StringBuilder builder = new StringBuilder();
    builder.append("problem_size,timing_ns");
    for (int i = 0; i < this.problemSizes.size(); i++) {
      builder.append('\n');
      builder.append(this.problemSizes.get(i));
      builder.append(",");
  ...
  /**
   * Write the timing experiment results to a txt file.
   *
   * @param filename - the filename to which the results will be written
   * Filename must contain the file extension, For this method it must be .txt
   */
  public void writeToTXT(String filename) {
    if (!filename.endsWith(".txt")) throw new IllegalAccessError("Invalid file extension, .txt expected");
  ...
  /**
   * Write the timing experiment results to a csvfile.
   *
   * @param filename - the filename to which the results will be written
   * Filename must contain the file extension, For this method it must be .csv
   */
  public void writeToCSV(String filename) {
    if (!filename.endsWith(".csv")) throw new IllegalAccessError("Invalid file extension, .csv expected");
  ...
  /**
   * Compute and return the median time elapsed to run the computation for a problem size.
   *
   * @param problemSize - the problem size for one experiment
  ...
  /**
   * Compute and return the elapsed time to run the computation for a problem size.
   *
   * @param problemSize - the problem size for one experiment
